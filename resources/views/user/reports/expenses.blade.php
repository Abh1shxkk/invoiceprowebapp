...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Track your business expenses by category</p>
        </div>
        <a href="{{ route('user.reports.expenses.pdf', ['start_date' => $startDate, 'end_date' => $endDate]) }}" 
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <form action="{{ route('user.reports.expenses') }}" method="GET" class="flex flex-wrap gap-4 items-end">
            <div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <p class="text-3xl font-bold text-red-600">${{ number_format($report['total_expenses'], 2) }}</p>
    </div>

    <!-- Category Breakdown -->
    @if($report['category_data']->count() > 0)
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200">
                <h4 class="text-base font-semibold text-gray-900">Expenses by Category</h4>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($report['category_data'] as $data)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $data['category'] }}</td>
                                <td class="px-6 py-4 text-sm text-right text-red-600 font-semibold">${{ number_format($data['total'], 2) }}</td>
                                <td class="px-6 py-4 text-sm text-right text-gray-600">{{ $data['count'] }}</td>
                                <td class="px-6 py-4 text-sm text-right text-gray-600">
                                    {{ $report['total_expenses'] > 0 ? number_format(($data['total'] / $report['total_expenses']) * 100, 1) : 0 }}%
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
        ...
        @if($report['monthly_data']->count() > 0)
            <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h4 class="text-base font-semibold text-gray-900">Monthly Expense Breakdown</h4>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-gray-50 border-b border-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($report['monthly_data'] as $data)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-sm text-right text-red-600 font-semibold">${{ number_format($data['expenses'], 2) }}</td>
                                    <td class="px-6 py-4 text-sm text-right text-gray-600">{{ $data['count'] }}</td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    @else
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p class="mt-4 text-sm text-gray-500">No expense data for the selected period</p>
        </div>
    @endif
</div>
@endsection
