...
<div class="max-w-5xl">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-600 mt-1">Update invoice details and line items</p>
        </div>

        <form action="{{ route('user.invoices.update', $invoice) }}" method="POST" id="invoiceForm" class="p-6 space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <div class="w-full md:w-1/2 space-y-3">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-2">
                                ...
                                <input type="number" name="tax" id="tax" value="{{ old('tax', $invoice->tax) }}" min="0" max="100" step="0.01"
            ...
            <div class="pt-6 border-t border-gray-200">
                ...
                <textarea name="notes" id="notes" rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-500 @enderror">{{ old('notes', $invoice->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addItem(itemData = null) {
        ...
        row.innerHTML = `
            <td class="px-4 py-2">
                ${itemId ? `<input type="hidden" name="items[${itemCount}][id]" value="${itemId}">` : ''}
